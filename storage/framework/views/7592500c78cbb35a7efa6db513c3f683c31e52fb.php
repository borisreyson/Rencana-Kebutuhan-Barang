...
<table class="table table-striped table-bordered">
  ...
  <tbody>
    <?php if(count($method)>0): ?>
    <?php $__currentLoopData = $method; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <tr>
      <td><?php echo e($v->code_methode); ?></td>
      <td><?php echo e($v->code_desc); ?></td>
      <td>
        <?php if($v->status=='0'): ?>
        <a href="<?php echo e(url('/admin/inventory/method/status-'.bin2hex($v->code_methode).'-enable')); ?>" class="btn btn-xs btn-success">Enable</a>
        <?php elseif($v->status=='1'): ?>
        <a href="<?php echo e(url('/admin/inventory/method/status-'.bin2hex($v->code_methode).'-disable')); ?>" class="btn btn-xs btn-danger">Disable</a>
        <?php endif; ?>
      </td>
      <td>
        <button type="button" class="btn btn-xs btn-warning" data-id="<?php echo e(bin2hex($v->code_methode)); ?>" id="editMethod" data-toggle="modal" data-target="#myModal">Edit</button>
        <a href="<?php echo e(url('/admin/inventory/method/del-'.bin2hex($v->code_methode))); ?>" class="btn btn-xs btn-danger" id="delLoc">Delete</a>
      </td>
    </tr>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    <tr class="info">
      <td colspan="4">
       <b>Total Record : <?php echo e(count($method)); ?></b>
      </td>
    </tr>
    <?php else: ?>
    <tr>
...
<script>
  ...
  $(document).on("click","button[id=new_Method]",function(){
    $.ajax({
      type:"GET",
      url:"<?php echo e(url('/admin/inventory/method/new')); ?>",
  ...
  $(document).on("click","button[id=editMethod]",function(){
    ...
    $.ajax({
      type:"GET",
      url:"<?php echo e(url('/admin/inventory/method/edit')); ?>",
