...
<form class="form-horizontal form-label-left" action="<?php echo e(url('/inventory/stok/store')); ?>" method="post">
...
<script src="<?php echo e(asset('/vendors/validator/validator.js')); ?>"></script>
    ...
    <script>
      ...
      $("button[name=scanqr]").click(function(){
        window.open("<?php echo e(url('/QRSCAN/')); ?>", "MsgWindow", "width=300,height=300,top=300,left=600");
      });
      ...
      $(document).on("focus",'input[id=item]',function(){
            ...
            $("input[id=item]").eq(eq).devbridgeAutocomplete({ 
              serviceUrl: "<?php echo e(url('/inventory/get/master/item')); ?>",
    ...
    // initialize the validator function
      ...
      $('form').submit(function(e) {
        ...
        return '<tr class="dynamic_item">'+
          ...
          '<?php $__currentLoopData = $master; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kk => $vv): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>'+
          '<option value="<?php echo e($vv->item); ?>">(<?php echo e($vv->item); ?>) <?php echo e($vv->item_desc); ?> <?php echo e($vv->part_number); ?></option>'+
          ...
          '<option value="">--PILIH--</option>'+
          '<?php $__currentLoopData = $lokasi; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kL => $vL): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>'+
          '<option value="<?php echo e($vL->code_loc); ?>"><?php echo e(strtoupper($vL->location)); ?></option>'+
          '<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>'+
          '</select>'+
          '</td>'+
          '<td>'+
          ...
          '<option value="">--PILIH--</option>'+
          '<?php $__currentLoopData = $kondisi; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kK => $vK): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>'+
          '<option value="<?php echo e($vK->code); ?>"><?php echo e($vK->code_desc); ?></option>'+
          '<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>'+
          '</td>'+
          '<td>'+
          '<select name="vendor[]" id="vendor" class="form-control" data-live-search="true">'+
          '<option value="">--PILIH--</option>'+
          '<?php $__currentLoopData = $vendor; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kV => $vV): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>'+
          '<option value="<?php echo e($vV->nama_supplier); ?>"><?php echo e(strtoupper($vV->nama_supplier)); ?></option>'+
          '<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>'+
          '</td>'+
          '<td><textarea name="desk[]" id="desk" class="form-control"></textarea></td>'+
          '<td><input type="text" name="tgl[]" class="form-control datepicker" value="<?php echo e(date("d F Y")); ?>"></td>'+
...
$("#modalKONTEN").on("focus",".child",function() {
  $('.staticParent').on('keydown', '.child', function(e){-1!==$.inArray(e.keyCode,[190,46,8,9,27,13,110])||(/65|67|86|88/.test(e.keyCode)&&(e.ctrlKey===true||e.metaKey===true))&&(!0===e.ctrlKey||!0===e.metaKey)||35<=e.keyCode&&40>=e.keyCode||(e.shiftKey||48>e.keyCode||57<e.keyCode)&&(96>e.keyCode||105<e.keyCode)&&e.preventDefault()});
    ...
    $("select[id=method]").change(function() {
      ...
      $.ajax({
        type:"POST",        
        url:"<?php echo e(url('/inventory/stock/fade')); ?>",
        data:{metode:dipilih},
...
$("#modalKONTEN").on("click","input[id=no_rkb]",function(){
  ...
  var myWindow = window.open("<?php echo e(url('/inventory/stok/masuk/list-')); ?>"+eq,"", "toolbar=no,scrollbars=yes,resizable=no,width="+(window.screen.availWidth)+",height="+(window.screen.availHeight));
});
$("#modalKONTEN").on("change","select[id=item]",function(){
  ...
  $.ajax({
    type:"GET",
    url:"<?php echo e(url('/inventory/master/item/list-')); ?>"+eq+"?item="+item,
...
eq = $("select[id=no_rkb]").index(this);
  ...
  $.ajax({
    type:"GET",
    url :"<?php echo e(url('/inventory/compare/quantity')); ?>",
...
$("#modalKONTEN").on("click","input[id=item]",function(){
  eq = $("input[id=item]").index(this);
  var myWindow = window.open("<?php echo e(url('/inventory/master/item/list-')); ?>"+eq,"", "toolbar=no,scrollbars=yes,resizable=no,top=200,left=350,width=800,height=400");
});
*/
$("#modalKONTEN").on("click","input[id=lokasi]",function(){
  eq = $("input[id=lokasi]").index(this);
  var myWindow = window.open("<?php echo e(url('/inventory/master/lokasi/list-')); ?>"+eq,"", "toolbar=no,scrollbars=yes,resizable=no,top=200,left=350,width=800,height=400");
});
$("#modalKONTEN").on("click","input[id=vendor]",function(){
  eq = $("input[id=vendor]").index(this);
  var myWindow = window.open("<?php echo e(url('/inventory/master/vendor/list-')); ?>"+eq,"", "toolbar=no,scrollbars=yes,resizable=no,top=200,left=350,width=800,height=400");
});
$("#modalKONTEN").on("click","input[id=kondisi]",function(){
  eq = $("input[id=kondisi]").index(this);
  var myWindow = window.open("<?php echo e(url('/inventory/master/kondisi/list-')); ?>"+eq,"", "toolbar=no,scrollbars=yes,resizable=no,top=200,left=350,width=800,height=400");
});
        ...
        $(document).ready(function(){
          ...
          $(document).on("focus",'input[id=location]',function(){
            ...
            $("input[id=location]").eq(eq).devbridgeAutocomplete({ 
              serviceUrl: "<?php echo e(url('/inventory/get/master/location')); ?>",
              onSelect: function (suggestion) {
                $("input[id=location_hide]").eq(eq).val(suggestion.data);
              },
              autoSelectFirst:true
          ...
          $(document).on("focus",'input[id=suplier]',function(){
            ...
            $("input[id=suplier]").eq(eq).devbridgeAutocomplete({ 
              serviceUrl: "<?php echo e(url('/inventory/get/master/suplier')); ?>",
              onSelect: function (suggestion) {
          ...
          $(document).on("focus",'input[id=category]',function(){
           ...
           $("input[id=category]").eq(eq).devbridgeAutocomplete({ 
              serviceUrl: "<?php echo e(url('/inventory/get/master/category')); ?>",
              onSelect: function (suggestion) {
          ...
          $(document).on("focus",'input[id=condition]',function(){
           ...
           $("input[id=condition]").eq(eq).devbridgeAutocomplete({ 
              serviceUrl: "<?php echo e(url('/inventory/get/master/condition')); ?>",
              onSelect: function (suggestion) {
...
$(document).on("focus",".child",function() {
  $('.staticParent').on('keydown', '.child', function(e){-1!==$.inArray(e.keyCode,[46,8,9,27,13,110])||(/65|67|86|88/.test(e.keyCode)&&(e.ctrlKey===true||e.metaKey===true))&&(!0===e.ctrlKey||!0===e.metaKey)||35<=e.keyCode&&40>=e.keyCode||(e.shiftKey||48>e.keyCode||57<e.keyCode)&&(96>e.keyCode||105<e.keyCode)&&e.preventDefault()});
        ...
        $("button[id=btn_row]").click(function() {
          ...
          //}
          if(no_rkb==""){
         var myWindow = window.open("<?php echo e(url('/get/rkb/details.popup-html')); ?>?no_rkb="+no_rkb+"&part_name="+values,"", "toolbar=no,scrollbars=yes,resizable=no,top=200,left=500,width=600,height=400");
          }else{
         var myWindow = window.open("<?php echo e(url('/get/rkb/details.popup')); ?>?no_rkb="+no_rkb+"&part_name="+values,"", "toolbar=no,scrollbars=yes,resizable=no,top=200,left=500,width=600,height=400");
          }
        });
...
$("button[name=addRKB]").click(function(){ 
  var myWindow = window.open("<?php echo e(url('/inventory/popup/detRKB')); ?>","", "toolbar=no,scrollbars=yes,resizable=no,top=200,left=500,width=600,height=400");
});
    ...
    function init_autocomplete() {      
      ...
      $('#no_rkb').devbridgeAutocomplete({ 
        ...
        onSelect: function (suggestion) {
                $.ajax({
                    ...
                    url:"<?php echo e(url('/get/rkb/details.complate')); ?>",
                    beforeSend:function(){
      ...
      $('input[id=part_number]').devbridgeAutocomplete({
        serviceUrl: "<?php echo e(url('/get/part/number')); ?>?no_rkb="+_norkb+"&part_name="+_part_name,
              autoSelectFirst:true
      });
      });
