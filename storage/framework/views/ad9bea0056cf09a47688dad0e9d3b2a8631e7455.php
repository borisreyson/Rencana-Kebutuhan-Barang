...
$(document).on("click","button[id=btn_row]",function(){
  ...
  $("tr[id=new_row]").before("<tr>"+
               ...
               "<?php $__currentLoopData = $master; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kM => $vM): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>"+
               "<option value=\"<?php echo e($vM->item); ?>\"><?php echo e($vM->item_desc.' '.$vM->part_number); ?></option>"+
               "<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>"+
               "</select>"+
               '<input type="hidden" name="item[]" id="item" class="form-control" placeholder="Item" style="height:41px!important;" readonly>'+
               "</td>"+
               "<td><input type=\"text\" name=\"part_number[]\" id=\"part_number\" style=\"height:41px!important;\" class=\"form-control\" placeholder=\"Part Number\" readonly></td>"+
               "<td>"+
...
$(document).on("click","button[id=attch_btn]",function(){
  ...
  $("#konten_modal").html('<div class="modal-content">'+
           '<div class="modal-header">'+
            ...
            "<form action=\"<?php echo e(asset('/v1/file-upload')); ?>\" class=\"form-horizontal dropzone\" id=\"myAwesomeDropzone\" data-tooltip=\"tooltip\" title=\"Maksimal yang di unggah adalah 9\">'"+
...
function soft_delete(vars){
        $.ajax({
                type:"POST",
                data:{_token:"<?php echo e(csrf_token()); ?>",token_:vars},
                url:"<?php echo e(asset('/v1/soft-delete')); ?>",
...
function fast_delete(varID){
  $.ajax({
    type:"POST",
    data:{_token:"<?php echo e(csrf_token()); ?>",keyID_:varID},
    url:"<?php echo e(url('/v1/fast-delete')); ?>",
    success:function(result){
      console.log(result);
    }
  });
...
$(document).on("focus",".child",function() {
  $('.staticParent').on('keydown', '.child', function(e){-1!==$.inArray(e.keyCode,[46,8,9,27,13,110])||(/65|67|86|88/.test(e.keyCode)&&(e.ctrlKey===true||e.metaKey===true))&&(!0===e.ctrlKey||!0===e.metaKey)||35<=e.keyCode&&40>=e.keyCode||(e.shiftKey||48>e.keyCode||57<e.keyCode)&&(96>e.keyCode||105<e.keyCode)&&e.preventDefault()});
    ...
    $(document).on("click","button[id=remove]",function(){
      ...
      if(part_name!=""){
      $.post("<?php echo e(url('/api/delete/entry')); ?>",{_token:"<?php echo e(csrf_token()); ?>",id_rkb:id_tmp},function(result){
        if(result=="OK"){
...
new PNotify({
    ...
    function loadData() {
      ...
      ?>
      //console.log("<?php echo e($v->quantity); ?>");
      $("tr[id=new_row]").before("<tr>"+
               '<td>'+
               '<input type="hidden" name="id_tmp[]" id="id_tmp" class="form-control id_tmp" value="<?php echo e($v->id_rkb); ?>" required>'+
               '<select name="part_name[]" data-live-search="true" id="part_name" class="form-control" placeholder=\"Part Name\" required>'+
               ...
               '<?php if($v->item==$vv->item): ?>'+
               '<option value="<?php echo e($vv->item); ?>" selected="selected"><?php echo e(($vv->item_desc)); ?> <?php echo e($vv->part_number); ?></option>'+
               '<?php else: ?>'+
               '<option value="<?php echo e($vv->item); ?>"><?php echo e(($vv->item_desc)); ?> <?php echo e(($vv->part_number)); ?></option>'+
               '<?php endif; ?>'+
               '<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>'+
               '</select>'+
               '</td>'+
               '<input type="hidden" name="item[]" id="item" class="form-control" placeholder="Item" style="height:41px!important;" value="<?php echo e(($v->part_name)); ?>" readonly>'+
               '<td><input type="text" name="part_number[]" id="part_number" style="height:41px!important;" class="form-control" placeholder="Part Number" value="<?php echo e(($v->part_number)); ?>" readonly></td>'+
               '<td>'+
               '<div class="col-xs-6" role="group">'+
               '<span class="staticParent">'+
               '<input type="number" min="1" step="0.1" style="width:65px;height:41px!important;" name="quantity[]" class="form-control child"  value="<?php echo e(($v->quantity)); ?>"></span>'+
               '</div>'+
               '<div class="col-xs-6 row" style="margin:0px!important;padding:0px!important;">'+
               '<input type="text" name="satuan[]" id="satuan" style="height:41px!important;"  class="form-control" readonly="readonly"  value="<?php echo e(($v->satuan)); ?>" required>'+
               '</div>'+
               '</td>'+
               '<td align="right"><input type="text" name="due_date[]" class="form-control datepicker" placeholder="Due Date" readonly="readonly" value="<?php echo e((date('d F Y ',strtotime($v->due_date)))); ?>"></td>'+
               '<td><textarea rows="1" name="remark[]" class="form-control" placeholder="Remark">'+
               `<?php echo strip_tags($v->remarks); ?>`+
               '</textarea></td>'+
               '<td><button type="button" id="attch_btn" class="btn btn-default" style="border:0px!important;" title="Attach File" ><i class="fa fa-2x fa-paperclip"></i></button></td>'+
               '<td><div class="form-control-static"><button type="button" id="remove" class="btn btn-xs btn-danger remove"><i class="fa fa-times"></i></button></div></td>'+
               '</tr>');
    ...
    $("button[id=edit]").on("click",function(){
      ...
      $.ajax({
        type:"POST",
        url:"<?php echo e(url('/form_rkb/img-edit.py')); ?>",
...
$(document).on("change","select[id=part_name]",function(){
    ...
    $.ajax({
      type:"GET",
      url:"<?php echo e(url('/rkb/get/master/part_number')); ?>",
