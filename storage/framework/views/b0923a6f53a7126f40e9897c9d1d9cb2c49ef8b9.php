...
<div class="col-lg-12 row">
          <?php if(in_array('user form',$arrRULE)): ?>
  <button class="btn btn-primary" id="newOutForm" data-toggle="modal" data-target="#myModal">Form Sarana Keluar</button>
  <button class="btn btn-primary" id="newIzinForm" data-toggle="modal" data-target="#myModal">Form Izin Keluar</button>
          <?php endif; ?>
    ...
    <div class="col-lg-6 col-xs-12">
      <table>
        <tr>
          ...
          <td>&nbsp;&nbsp;&nbsp; &nbsp;</td>
          <td>
            Pemohon <font color="blue" style="font-weight: bolder;"> <?php echo e(ucwords($pemohon->nama)); ?></font>
          </td>
        </tr>
        <tr>
          <td>&nbsp;</td>
          ...
          <td>
            <span>
<?php if(!isset($_GET['dt_expr'])): ?>
          <?php if($v->flag_appr==2 && $v->flag==0): ?>
          <?php if($v->no_lv=="motor" || $v->no_lv=="mobil"): ?>
              <a href="" id="editMotor" data_id="<?php echo e($v->noid_out); ?>" class="btn btn-xs btn-warning" >Edit</a>
          <?php else: ?>
              <a href="" id="edit" data_id="<?php echo e($v->noid_out); ?>" class="btn btn-xs btn-warning" >Edit</a>
          <?php endif; ?>
              <a href="" id="cancel"  data_id="<?php echo e($v->noid_out); ?>" class="btn btn-xs btn-danger" >Cancel</a>
              <?php endif; ?>
<?php endif; ?>
            </span>
          </td>
      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="col-lg-12">
            ...
            <?php else: ?>
          <b class="col-lg-2">No LV </b> <span class="col-lg-9"><b>:</b> <?php echo e($v->no_lv); ?></span>
          <?php endif; ?>
          </div>
          <div class="col-lg-12">
            <?php if($v->no_lv=="motor" || $v->no_lv=="mobil"): ?>
...
<script>
  ...
  $("a[id=editMotor]").click(function(){
    eq = $("a[id=editMotor]").index(this);
    noid_out = $("a[id=editMotor]").eq(eq).attr("data_id");
    window.open("<?php echo e(url('/sarpras/sarana/keluar-masuk/Motor/edit')); ?>"+noid_out,"_blank","width="+screen.width+",height="+screen.height+",top=50,left=200");
    return false;
  });

  $("a[id=cancel]").click(function(){
    eq = $("a[id=cancel]").index(this);
    noid_out = $("a[id=cancel]").eq(eq).attr("data_id");
    window.open("<?php echo e(url('/sarpras/sarana/keluar-masuk/cancel')); ?>"+noid_out,"_blank","width="+(screen.width/2)+",height="+(screen.height/2)+",top=50,left=200");
    return false;
  });
  var eventDates = {};
  <?php
  ...
  $("button[id=newIzinForm]").click(function(){
    $.ajax({
      type:"GET",
      url :"<?php echo e(url('/sarpras/sarana/form-izin-keluar')); ?>",
      beforeSend:function(){
        $("div[id=modal_dialog]").removeClass("modal-md").addClass("modal-lg");
      },
      success:function(res){
