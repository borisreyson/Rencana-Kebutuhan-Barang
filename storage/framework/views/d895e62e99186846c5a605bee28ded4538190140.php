...
<div class="right_col" role="main">
  <div class="col-lg-12">
    ...
    <a href="<?php echo e(url('/hauling')); ?>">HAULING</a>
    <br>
    <br>
  </div>
  <div class="row">
...
function Getnoldua($pieces)
  ...
  foreach ($data as $key => $value) {
    ...
    if($value->plan_daily==0 && $value->actual_daily==0){
      $actual[] =  (float) Getnoldua(number_format($value->plan_daily,2));
    }elseif($value->plan_daily==0 && $value->actual_daily!=0){
      $actual[] =  (float) Getnoldua(number_format(1,2)*100);
    }else{
  ...
  foreach ($data_Y as $key => $value) {
    ...
    //$plan[] = $value->mtd_plan;
    if($value->mtd_plan==0 && $value->mtd_actual==0){
      $actualM[] =  (float) Getnoldua(number_format($value->mtd_plan,2)); 
    }elseif($value->mtd_plan==0 && $value->mtd_actual!=0){     
      $actualM[] =  (float) Getnoldua(1*100);
    }else{
