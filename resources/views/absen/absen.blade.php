...
<div class="col-lg-12 row">
    ...
    <div class="col-lg-6">
      <select name="nik" id="nik"  class="form-control" required="required" data-live-search="true">
        ...
        ?>
          ...
          @if(isset($_GET['nik']))
          @if($_GET['nik']==$vv->nik)
          <option value="{{$vv->nik}}" selected="selected">({{$vv->nik}}) {{$vv->nama}}</option>  
          @else          
          @if(isset($cek->nik) == $vv->nik)
          <option value="{{$vv->nik}}" class="label-success">({{$vv->nik}}) {{$vv->nama}} </option>
          @else          
          <option value="{{$vv->nik}}">({{$vv->nik}}) {{$vv->nama}}</option> 
          @endif   
          @endif
          @else        
          @if(isset($cek->nik) == $vv->nik)
  ...
  <div class="col-lg-12">
    ...
    <div class="col-lg-12">
      <a href="{{url('/absen/user/hge/export?').$_SERVER['QUERY_STRING'].'&dept='.$dept}}" class="btn btn-primary">Export</a>
    </div>
    ...
    <div class="col-lg-6 listPerson">
      ...
      <div class="col-lg-12 row">
        <div class="col-lg-3 row">
          <a href="{{url('/face_id').'/'.$v->nik.'/'.$v->gambar}}" target="_blank" title="Click to Zoom"><img class="thumbnail" style="margin: 0px!important;" src="{{asset(url('/face_id').'/'.$v->nik.'/'.$v->gambar)}}"></a></div>
