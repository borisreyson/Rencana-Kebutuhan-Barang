...
@if($value->cancel_by==null)
                            ...
                            <td style="text-align: left;">
                              <b>{{$value->no_po}}</b> <br><br>
                                <p>{{$value->keterangan}}</p><br><br>
                               @if(isset($value->time_input))
                               {{date("H:i:s",strtotime($value->time_input))}}<br>
                               {{date("d F Y",strtotime($value->time_input))}}@endif<br>
                               <label> {{$value->user_input}}</label></td>
                          </tr>
@endforeach
    ...
    "order": [[ 0, "desc" ]],
     ...
     buttons: [
     {
          ...
          className: "btn-sm btn-export",
             ...
             {
               extend: "csv",
               title: "Data export e-RKB From {{$_GET['startDate']}} - {{$_GET['endDate']}}"
             },
             {
               extend: "excel",
               title: "Data export e-RKB From {{$_GET['startDate']}} - {{$_GET['endDate']}}"
