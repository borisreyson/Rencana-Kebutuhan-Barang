    ...
    public function dataKaryawan(Request $request)
    {
        ...
        if(isset($_GET['status'])){
            if($_GET['status']=="aktif"){
                $status =$karyawan->where("flag",0);
            }else if($_GET['status']=="tidak_aktif"){
                $status =$karyawan->where("flag",1);
            }
        }else{
            $status =$karyawan;
        }
        ...
        if(isset($_GET['cari'])){
            ...
            $filter = $status->whereRaw("
                        db_karyawan.data_karyawan.nik like '%".$cari."%' or 
                        db_karyawan.data_karyawan.nama like '%".$cari."%' or 
                        department.dept like '%".$cari."%' or 
                        db_karyawan.data_karyawan.jabatan like '%".$cari."%'");
        }else{
            $filter = $status;
        }
