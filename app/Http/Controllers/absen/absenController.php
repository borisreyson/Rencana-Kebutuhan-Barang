    ...
    {        
      // dd($_SESSION['department']);
        ...
        $dbKaryawan = DB::table("db_karyawan.data_karyawan")->where("flag",0);
          ...
          $data = $filter->paginate($hal);
        return view('absen.rekap',["getUser"=>$this->user,"dbKaryawan"=>$data,"dept"=>$dept]); 
  ...
  // die();
    ...
    public function lastAbsen(Request $request)
    {
        ...
        if(isset($last)){
          ...
          if(isset($lastNew)){
            if(isset($roster)){
              return array("lastAbsen"=>$last->status,"lastNew"=>$lastNew->status,"tanggal"=>$lastNew->tanggal,"masuk"=>$roster->masuk,"pulang"=>$roster->pulang);
            }else{
              return array("lastAbsen"=>$last->status,"lastNew"=>$lastNew->status,"masuk"=>null,"pulang"=>null);
            }
          }else{
              return array("lastAbsen"=>$last->status,"lastNew"=>null,"masuk"=>null,"pulang"=>null);
          }
        }else{
          if(isset($lastNew)){
            if(isset($roster)){
              ...
              return array("lastAbsen"=>null,"lastNew"=>$lastNew->status,"masuk"=>date("H:i:s",$masuk),"pulang"=>$pulangNew);
              ...
              return array("lastAbsen"=>null,"lastNew"=>$lastNew->status,"masuk"=>null,"pulang"=>null);
            }

        }else{
          return array("lastAbsen"=>null,"lastNew"=>null,"masuk"=>null,"pulang"=>null);
        }
...
return redirect()->back()->with("failed","Periksa Bulan di File Anda");
    ...
    public function formKaryawan(Request $request)
    {
      ...
      return view('absen.form_karyawan',["getUser"=>$this->user,"dep"=>$dep]);
    }
    ...
    public function editKaryawan(Request $request)
    {
      ...
      $getKar = DB::table("db_karyawan.data_karyawan")->where("nik",$nik)->first();
      return view('absen.form_karyawan',["getUser"=>$this->user,"dep"=>$dep,"getKar"=>$getKar]);
